...
public class Commands {
    ...
    static {
        ...
        }.runTaskTimer(main.Main.me, 432000, 432000);
    }
        ...
        switch (label) {
            ...
            case "namechange":
                if (sender instanceof Player) {
                    ...
                    if (!main.Main.me.prefixes.containsKey(sender.getName())) {
                        sender.sendMessage(ChatColor.RED + "You don't have access to this command!");
                        return true;
                    }
    ...
    private static boolean leaveCommand(CommandSender sender) {
        ...
        MazeInstance inst = main.Main.me.getInstance(p);
        if (inst == null) {
        ...
        inst.checkEnd(false);
        p.teleport(main.Main.me.lob.getSpawn());
    ...
    private static boolean nameChangeCommand(String name, String prfx) {
        ...
        main.Main.me.prefixes.put(name, prfx);
        return true;
    }
    ...
    private static boolean kitCommand(CommandSender sender, String arg) {
        ...
        Player p = (Player) sender;
        if (arg == null) {
            p.sendMessage(ChatColor.GREEN + "Available kits: " + main.Main.kitnames.keySet().toString());
            return true;
        }
        if (!main.Main.kitnames.containsKey(arg.toLowerCase())) {
            p.sendMessage(ChatColor.RED + "That's not a kit!");
            return true;
        }
        main.Main.me.chosenKits.put(p, main.Main.kitnames.get(arg.toLowerCase()));
        p.sendMessage(ChatColor.GREEN + "You have chosen the " + main.Main.me.chosenKits.get(p).getName() + " kit!");
        return true;
    }
    ...
    private static boolean gameChatCommand(String name, String[] args) {
        ...
        String pfx = main.Main.me.prefixes.containsKey(name) ? main.Main.me.prefixes.get(name) : "";
        ...
        for (Player p : main.Main.me.players) {
    ...
    private static boolean premiumAllowCommand(String[] args) {
        ...
        YamlConfiguration prems = main.Main.me.prems;
    ...
    public static void saveReportsToFile() {
        try {
            ...
            File reportdata = new File(main.Main.me.getDataFolder(), datetime + "reports.txt");
