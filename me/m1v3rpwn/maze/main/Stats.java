...
public class Stats {
    ...
    public static void incrementWins(Player p, boolean single) {
        ConfigurationSection wins = me.m1v3rpwn.maze.main.Main.me.stats.getConfigurationSection(single ? "singlewins" : "wins");
        if (wins == null) {
            wins = me.m1v3rpwn.maze.main.Main.me.stats.createSection(single ? "singlewins" : "wins");
        }
        int win = 0;
    ...
    public static void incrementGames(Player p, boolean single) {
        ConfigurationSection wins = me.m1v3rpwn.maze.main.Main.me.stats.getConfigurationSection(single ? "singlegames" : "games");
        if (wins == null) {
            wins = me.m1v3rpwn.maze.main.Main.me.stats.createSection(single ? "singlegames" : "games");
        }
        int game = 0;
        if (!wins.contains(p.getName())) {
            wins.set(p.getName(), 1);
